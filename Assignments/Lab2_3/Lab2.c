/*  Names: Eric Segerstrom, Justin Conroy, 
    Section: 2
    Date: 10.1.2108
    File name: Lab2.c
    Description: Lab 2
...
void CompareVals(char input, char expected); // compares given answer to desired value and lights BILE
...
void ADC1_Init(void)
{
	...
	REF0CN = 0x03;		// sets internal voltage reference to 2.4V
	ADC1CF |= 0x01;		// sets converter gain to 1
	ADC1CF &= 0xFD;		
	ADC1CN = 0x80;		// enables converter
...
{
	AMX1SL = 0;		// sets P1.0 as analog input for ADC1
	ADC1CN = ADC1CN & ~0x20;	// clears conversion complete flag
	ADC1CN = ADC1CN | 0x10;		// starts conversion
	while ((ADC1CN & 0x20) == 0x00);	// waits for conversion to complete
	return ADC1;
}
...
{	
	...
	for (rounds = 0; rounds < 5; rounds++ )
	{
		...
		round_Score = (game_Time - 169)/169; //The first half second does not count
		if (error == 1)
		{
...
void Mode_Three(void)
{
	...
	for (round = 0; round<=5; round++) {
		...
		CompareVals(MapADC(ADC_value, 0, 63), desiredValue);

		error = (abs(desiredValue - MapADC(ADC_value, 0, 63))*2);	// calculates error
		...
		totalPoints = totalPoints + score;		// calculates total score
